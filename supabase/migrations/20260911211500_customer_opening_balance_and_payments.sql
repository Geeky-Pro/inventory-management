...
CREATE OR REPLACE FUNCTION public.add_customer_opening_balance(
  _operation_id UUID,_customer_id UUID,_amount_local NUMERIC,_currency_code TEXT,
  _exchange_rate NUMERIC,_transaction_date DATE,_notes TEXT DEFAULT NULL
...
BEGIN
 ...
 IF _operation_id IS NULL OR _customer_id IS NULL OR _amount_local<=0 OR _exchange_rate<=0 THEN RAISE EXCEPTION 'Invalid opening balance'; END IF;
 ...
 SELECT id INTO v_id FROM public.debt_transactions WHERE reference_table='customer_opening_balances' AND reference_id=_operation_id AND customer_id=_customer_id LIMIT 1;
 IF v_id IS NOT NULL THEN RETURN v_id; END IF;
 INSERT INTO public.debt_transactions(customer_id,transaction_type,amount,currency_code,exchange_rate,amount_local,reference_table,reference_id,notes,transaction_date,created_by)
 VALUES(_customer_id,'debit',_amount_local/_exchange_rate,_currency_code,_exchange_rate,_amount_local,'customer_opening_balances',_operation_id,_notes,_transaction_date,v_user)
...
BEGIN
 ...
 IF v_id IS NOT NULL THEN RETURN v_id; END IF;
 SELECT COALESCE(SUM(CASE WHEN transaction_type='debit' THEN amount_local ELSE -amount_local END),0) INTO v_balance
 FROM public.debt_transactions WHERE customer_id=_customer_id;
 ...
 VALUES(_customer_id,'credit',_amount_local/_exchange_rate,_currency_code,_exchange_rate,_amount_local,'customer_payments',_operation_id,_payment_method,_notes,_transaction_date,v_user)
 RETURNING id INTO v_id; RETURN v_id;
END $$;
...
GRANT EXECUTE ON FUNCTION public.add_customer_opening_balance(UUID,UUID,NUMERIC,TEXT,NUMERIC,DATE,TEXT) TO authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.record_customer_payment(UUID,UUID,NUMERIC,TEXT,NUMERIC,DATE,TEXT,TEXT) TO authenticated,service_role;
